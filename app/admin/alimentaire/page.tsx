...
// Nombre de bilans créés cette semaine, tous clients confondus
...
// Moyenne d'activité calculée sur le dernier bilan de chaque client
...
export default async function AlimentairePlanPage() {
  ...
  try {
    usersWithProfiles = await prisma.user.findMany({
      ...
      orderBy: {
        // Trie les clients par date du bilan le plus récent
        alimentaireProfiles: {
          _count: "desc",
        },
      },
    ...
    // Tri secondaire par date du dernier bilan (le plus récent en premier)
                    ...
                    return (
                      <TableRow key={u.id} className="hover:bg-muted/50">
                        ...
                        <TableCell className="text-center">
                          <Link href={`/alimentaire/client/${u.id}`}>
                            <Button
                              variant="outline"
                              size="sm"
